...
@section('content')
    ...
    <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
        <a href="{{route('dealCreate')}}" class="button text-white bg-theme-1 shadow-md mr-2">გარიგების ტიპის დამატება</a>
    </div>
    <div class="intro-y datatable-wrapper box p-5 mt-5">
        <table class="table table-report table-report--bordered display datatable w-full">
            ...
            @foreach($deals as $deal)
                <tr>
                    <td class="border-b">
                        <div class="text-gray-600 text-xs whitespace-no-wrap">{{ $deal->id }}</div>
                    </td>
                    <td class="border-b">
                        <div class="flex items-center sm:justify-center "> {{ $deal->title_ge }}</div>
                    </td>
                    <td class="border-b">
                        <div class="flex items-center sm:justify-center "> {{ $deal->title_en }}</div>
                    </td>
                    <td class="w-40 border-b">
                        @if($deal->status)
                            <a href={{route('dealActivate',$deal->id)}} class="flex items-center sm:justify-center text-theme-9"> <i
                                        data-feather="check-square" class="w-4 h-4 mr-2"></i> აქტიური</a>
                        @else
                            <a href={{route('dealActivate',$deal->id)}} class="flex items-center sm:justify-center text-theme-6"> <i
                                        data-feather="check-square" class="w-4 h-4 mr-2"></i> არა აქტიური </a>
                        @endif
                    </td>
                    <td class="border-b w-5">
                        <div class="flex sm:justify-center items-center">
                            <a class="flex items-center mr-3" href={{route('dealUpdate',$deal->id)}}> <i
